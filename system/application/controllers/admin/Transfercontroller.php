		...
		if(count($getTransferHistory)>0)
		{
			...
			foreach($getTransferHistory as $rs)
			{
				...
				if(count($getReceiver)>0)
				{
					...
					{
						$receiver_name = $rs1->receiver_name;						$reciver_user_id = $rs1->reciver_user_id;						$receiver_mobile = $rs1->receiver_mobile;
					}
				}
				$listData .= '<tr><td>'.$i.'</td><td>'.$rs->sender_name.' ( '.$rs->sender_mobile.' ) <a href="'.base_url().admin.'/view-user/'.$rs->sender_user_id.'"><i class="bx bx-link-external"></i></a></td><td>'.$receiver_name.' ( '.$receiver_mobile.' ) <a href="'.base_url().admin.'/view-user/'.$reciver_user_id.'"><i class="bx bx-link-external"></i></a></td><td>'.$rs->amount.'</td><td>'.$rs->insert_date.'</td>
				</tr>';
				$i++;
			}
		}
